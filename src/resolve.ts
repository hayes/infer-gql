...
export type ShapeFromName<
  Types extends BaseTypes,
  Name extends string
> = Name extends keyof Types ? Types[Name] : {};

export type ShapeFromTypeRef<
  Types extends BaseTypes,
  Ref extends TypeRefNode
> = ShapeFromName<Types, Ref["name"]> extends infer Shape
  ? Ref extends { list: true }
    ? Ref extends { nonNull: true }
      ?
          | (Ref extends { listItemNonNull: true }
              ? Shape
  ...
  ? Tokens extends TypeNode[]
    ? {
        [K in Tokens[number] as K extends ObjectTypeNode ? K["name"] : never]: {
          [F in (K &
            ObjectTypeNode)["fields"][number] as F["name"]]: ResolverFromField<
            K["name"],
            F,
            Types,
            Context
        ...
        [K in Tokens[number] as K extends InterfaceTypeNode
          ? K["name"]
          : never]?: {
          [F in (K &
            InterfaceTypeNode)["fields"][number] as F["name"]]?: ResolverFromField<
            K["name"],
            F,
            Types,
            Context
...
export const resolvers: ResolveTypes<
  SDL,
  BaseTypes & { User: { id: number; name: string } }
> = {
  Query: {
    user: (parent, args, ctx, info) =>
      Promise.resolve({ id: 123, name: "Name" }),
